...
class ZeekLibrary extends ZeekOutput {
     ...
     * @param boolean/integer depth of scan
     * @param integer size to remove
     * @param array filter of directories
     * @param boolean get short path name
     */
    public function directory_scan($path, &$list, $depth, $len_to_remove=0,
	    ...
	    try
	    {
	        ...
	        foreach ($files as $file)
	        {
		        ...
		        if (filetype($filepath) == "dir")
		        {
                    ...
                    if ((is_numeric($depth) && --$depth >= 0) || $depth)
		                $this->directory_scan(
			                $filepath, $list, $depth,
     ...
     * @param depth of scan
     */
    public function contents_get_list($project_id, $directory_name, $depth=false)
    {
	    $rsp = array();

        $src = $this->global_path . "projects/$project_id/$directory_name";

	    if (is_dir($src) && $this->directory_scan($src, $rsp, $depth, -1, null, true))
	        return $rsp;
